...
class ListValue:
    ...
    def __init__(
        self, initial_value, available_values: List, select_closest: bool = False
    ):
...
class LimitValue:
    ...
    def value(self, new_value):
        if self.limits is not None and self.limits.min > new_value > self.limits.max:
            raise ValueError(
                f"The value {new_value} is outside the limits of {self.limits}"
            )
        self._value = new_value

    def __init__(self, initial_value, limits: Optional[Tuple] = None):
        self.limits = (
            LimitValue.Limits(min(limits), max(limits)) if limits is not None else None
        )
        self.value = initial_value
