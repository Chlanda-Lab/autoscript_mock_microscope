from autoscript_sdb_microscope_client.enumerations import BeamType, PatternScanDirection, PatternScanType
from random import randint
from typing import List

class RectanglePattern:
    def __init__(self,
            application_file: str, beam_type: int,
            center_x: float, center_y: float,
            depth: float, dwell_time: float,
            enabled: bool, height: float,
            rotation: float, scan_direction: str,
            scan_type: str, width: float):
...
class Patterning:
    ...
    def set_default_application_file(self, application_file: str):
        if application_file not in self.list_all_application_files():
            raise ValueError(f'Application file "{application_file}" not available in {self.list_all_application_files()}')
